...
def updateEqmasterlist():
    ...
    for i in range(master.shape[0]):
        session.add(eqmasterlist(Country=master.loc[i]['Country'], 
                                 Ticker=master.loc[i]['Ticker'], 
                                 Staging=master.loc[i]['Staging'], 
                                 Quandl_DB=master.loc[i]['Quandl_DB']))
        session.commit()